...
async function deleteSubnet(id, name) {
  if (!confirm(`Remove subnet "${name}"?\n\nThis will not delete any scanned hosts — only the subnet entry and its scan history.`)) return;
  await api("DELETE", `/api/subnets/${id}`);
  subnets = subnets.filter(s => s.id !== id);
  hiddenSubnets.delete(id);
  renderSubnetList();
}
...
function renderContainerPick() {
  ...
  if (containers.length === 0) {
    list.innerHTML = `<div style="font-size:11px;color:var(--text-3)">No container hosts available to import.</div>`;
    return;
  }
  list.innerHTML = `<div class="container-pick-grid">` +
    containers.map(h => {
      ...
      return `<div class="container-chip" onclick="pickContainer('${h.ip}','${label}')">
